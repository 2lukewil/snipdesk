  ...
  try {
    const picked = await withDialogSuppressed(() =>
      openDialog({
        ...
        filters: [
          {
            name: "Snippet files (PhraseExpress .pex XML, JSON, CSV)",
            extensions: ["pex", "json", "csv", "pxx", "xml"],
          },
          { name: "PhraseExpress XML export (*.pex)", extensions: ["pex", "pxx"] },
          { name: "JSON", extensions: ["json"] },
          { name: "CSV", extensions: ["csv"] },
          { name: "PhraseExpress database (*.pexdb) — may be encrypted", extensions: ["pexdb"] },
        ],
      })
    );
    ...
    else if (lower.endsWith(".pexdb")) format = "pexdb";
    else if (lower.endsWith(".pex") || lower.endsWith(".pxx") || lower.endsWith(".xml"))
      format = "pex";
    else format = "phraseexpress";
