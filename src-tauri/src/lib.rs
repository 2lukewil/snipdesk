...
mod tests {
    ...
    #[test]
    fn friendly_shortcut_normalizes_modifier_casing() {
        assert_eq!(friendly_shortcut("Ctrl+Shift+Space"), "Ctrl+Shift+Space");
        assert_eq!(friendly_shortcut("control+alt+f1"), "Ctrl+Alt+F1");
        assert_eq!(friendly_shortcut("ALT+SPACE"), "Alt+Space");
        assert_eq!(friendly_shortcut("shift+a"), "Shift+A");
    }

    #[test]
    fn friendly_shortcut_resolves_cmdorctrl_per_platform() {
        // On every supported OS exactly one of these should land.
        let out = friendly_shortcut("CommandOrControl+Shift+P");
        assert!(out == "Ctrl+Shift+P" || out == "Cmd+Shift+P");
    }

    #[test]
    fn friendly_shortcut_passes_through_pathological_input() {
        // Contract: never panic on malformed user input.
        assert_eq!(friendly_shortcut(""), "");
        assert_eq!(friendly_shortcut("+"), "");
        assert_eq!(friendly_shortcut("  "), "  ");
    }
    ...
    fn parse_shortcut_rejects_invalid_input() {
        ...
        // Modifier-only — there's no key code, so unwrap should be None.
        assert!(parse_shortcut("Ctrl+Shift").is_none());
    }

    #[test]
    fn code_from_str_handles_letters_digits_function_keys() {
        assert!(code_from_str("A").is_some());
        assert!(code_from_str("z").is_some());
        assert!(code_from_str("0").is_some());
        assert!(code_from_str("F12").is_some());
        assert!(code_from_str("Space").is_some());
        assert!(code_from_str("ENTER").is_some());
        assert!(code_from_str("escape").is_some());
    }

    #[test]
    fn code_from_str_rejects_unknown_keys() {
        assert!(code_from_str("NOTAKEY").is_none());
        assert!(code_from_str("").is_none());
        // F13+ aren't in the supported set.
        assert!(code_from_str("F13").is_none());
    }
}
