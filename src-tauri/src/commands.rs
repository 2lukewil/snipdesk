...
mod tests {
    ...
    fn unknown_placeholders_pass_through() {
        let v = vars(&[("name", "Alex")]);
        assert_eq!(
            substitute_variables("Hi {name}, your {service} expires soon", &v),
            "Hi Alex, your {service} expires soon"
        );
    }

    #[test]
    fn unicode_passes_through_unchanged() {
        // Regression guard. The byte-loop implementation pushed individual
        // UTF-8 bytes as Latin-1 chars, turning `—` (E2 80 94) into
        // `â` plus garbage. This test pins the fix.
    ...
    #[test]
    fn empty_braces_pass_through_verbatim() {
        let v: HashMap<String, String> = HashMap::new();
        assert_eq!(substitute_variables("Hello {} world", &v), "Hello {} world");
    }
    ...
    fn unclosed_brace_is_emitted_verbatim() {
        let v: HashMap<String, String> = HashMap::new();
        assert_eq!(
            substitute_variables("Use {name and go", &v),
            "Use {name and go"
        );
    }
    ...
    fn empty_value_substitutes_empty() {
        let v = vars(&[("middle", "")]);
        assert_eq!(substitute_variables("a{middle}b", &v), "ab");
    }

    #[test]
    fn invalid_placeholder_chars_are_left_alone() {
        // Spaces and dots aren't in the var-name grammar — keep the literal.
        let v = vars(&[("name with space", "X"), ("dotted.name", "Y")]);
        assert_eq!(
            substitute_variables("hi {name with space} {dotted.name}", &v),
            "hi {name with space} {dotted.name}"
        );
    }

    #[test]
    fn is_valid_var_name_accepts_safe_chars() {
        assert!(is_valid_var_name("customer_name"));
        assert!(is_valid_var_name("invoice-id"));
        assert!(is_valid_var_name("v1"));
        assert!(is_valid_var_name("X"));
    }

    #[test]
    fn is_valid_var_name_rejects_unsafe() {
        assert!(!is_valid_var_name(""));
        assert!(!is_valid_var_name("has space"));
        assert!(!is_valid_var_name("dot.name"));
        assert!(!is_valid_var_name("emoji😀"));
    }
}
